...
static void bmpFillColor(GBitmap *bmp, GColor c) {
  int pattern = 0, s;
  if (c == GColorWhite) {
    pattern = 0xff;
  }
  
  // Calc the size of the data array in bytes
  s = bmp->row_size_bytes * bmp->bounds.size.h;
  
  // Replace with pattern
  memset(bmp->addr, pattern, s);
}
...
static void bmpCopyTo(const GBitmap *src, GBitmap *dst, GPoint offset) {
  int sx, sy, dx, dy;
    for (sy=0, dy=offset.y; sy<src->bounds.size.h; sy++, dy++) {
        for (sx=0, dx=offset.x; sx<src->bounds.size.w; sx++, dx++) {
            bmpPutPixel(dst, dx, dy, bmpGetPixel(src, sx, sy));
        }
    }
}
...
static void bmpRotate(const GBitmap *src, GBitmap *dst, int a, GRect *srcClipRect, GPoint srcCenter, GPoint dstOffset) {
  ...
  GPoint srcCorner[4], dstCorner[4];
	...
	if (srcClipRect == NULL) {
		...
		srcCorner[1] = GPoint(src->bounds.size.w-1,0);
    srcCorner[2] = GPoint(src->bounds.size.w-1,src->bounds.size.h-1);
		srcCorner[3] = GPoint(0,src->bounds.size.h-1);
	} else {
		srcCorner[0] = GPoint(srcClipRect->origin.x, srcClipRect->origin.y);
		srcCorner[1] = GPoint(srcClipRect->origin.x + srcClipRect->size.w-1, srcClipRect->origin.y);
    srcCorner[2] = GPoint(srcClipRect->origin.x + srcClipRect->size.w-1, srcClipRect->origin.y + srcClipRect->size.h-1);
		srcCorner[3] = GPoint(srcClipRect->origin.x, srcClipRect->origin.y + srcClipRect->size.h-1);
	}
    ...
    //APP_LOG(APP_LOG_LEVEL_DEBUG, "dstCorner[%d] = (%d, %d)", i, dstCorner[i].x, dstCorner[i].y);
	...
	if (xmin < 0) xmin = 0;
	if (ymin < 0) ymin = 0;
	if (xmax > dst->bounds.size.w) xmax = dst->bounds.size.w - 1;
	if (ymax > dst->bounds.size.h) ymax = dst->bounds.size.h - 1;

  //APP_LOG(APP_LOG_LEVEL_DEBUG, "Min (%d, %d) - Max (%d, %d)", xmin, ymin, xmax, ymax);
	...
	for (y=ymin; y<=ymax; y++) {
		...
		yy = xs + yc;
		for (x=xmin; x<=xmax; x++) {
			c = bmpGetPixel(src, (int)(TRIG_NORM(xx) + srcCenter.x), (int)(TRIG_NORM(yy) + srcCenter.y));
